...
Every IP is a repository of the same shape: an `ip.yaml` that names its knobs, contracts and measured area, a `regmap.yaml` for its registers, Bluespec sources, and behaviour tests that run over its whole configuration matrix. [`xirang`](https://github.com/Tape-Out/xirang) resolves, generates, tests and prices them, and [`spec`](https://github.com/Tape-Out/spec) says what the files mean.
