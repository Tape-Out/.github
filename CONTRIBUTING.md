...
If you changed `bsv/`, `ip.yaml` or `regmap.yaml`, the price list no longer matches the design and `ran lint` says so. Measuring it again needs the ICS55 PDK, which public runners do not have. If you do not have it either, say so in the pull request.

## How IP is written here

- **BSV or BH by the nature of the block.** Rules, state and scheduling are written in BSV. Pure combinational tables, folds over vectors, type-level work and pattern matching are written in BH. Both live in `bsv/`.
